...
import { upload, validateAndSaveWebp } from "../middleware/uploads.middleware";

const router = Router();

router.use(   "/",      express.static(path.join(__dirname, "../../public/uploads")));
router.post(  "/",      authenticate, checkRole("admin"), upload, validateAndSaveWebp, UploadController.uploadFile);
